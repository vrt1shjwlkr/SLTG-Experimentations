...
# distance between each pair of coordinates in same time averaged over the total time duration
def distance_bw_traces(seed, fake, direct_dist, locations_array):
	if direct_dist:
		gen_distance = 0
		for i in range (len(seed)):
			distance = 1000 * vincenty(locations_array[seed[i]-1], locations_array[fake[i]-1]).kilometers
			gen_distance += distance/len(seed)
		return gen_distance
	else:
		gen_distance = 0
		for i in range (len(seed)):
...
# distance traveled by given trace
def trace_distance(fake, direct_dist, locations_array):
	if direct_dist:
		distance_covered = 0
		for i in range(len(fake)-1):
			distance_covered += 1000 * vincenty(locations_array[fake[i]-1], locations_array[fake[i+1]]).kilometers
...
def calculate_metrics():
	ar = open('avg_results.csv', 'w')
	ar_writer = csv.writer(ar)
	header = ['experiment', 'num of users', 'trace length', 'max location in trace', 'max location allowed', 'Clusters', 'GLRP', 'MP', 'ALRP', 'VF', 'User', 'avg_gen_loss', 'avg_dist_loss', 'avg_surge_loss', 'seed_std', 'avg_fake_std']
	ar_writer.writerow(header)
	for folder in sorted(os.listdir(experiment1_files)):
		...
		for config_path in sorted(os.listdir(experiment1_files + folder + '/')):
			...
			if config_path.startswith('out_'):
				# extract locations from locations file and put in locations_array
				locations_array = []
				if config[2] != '84':
					locations_array = None
				else:
					with open(experiment1_files + folder + '/' + config_path + '/locations', 'r') as lf:
						for i, line in enumerate(lf):
							locations_array.append([float(line.split(',')[1]), float(line.split(',')[0])])
				...
				with open('results_' + folder[5:] + config_path[3:] + '.csv', 'w') as rf:
					...
					row_to_ar = [experiment1_files.split('/')[-2], config[1], config[2], config[3], config[4], config[5]]
					row_to_ar.append(config_path.split('_')[1]) # GLRP				
					row_to_ar.append(config_path.split('_')[2]) # MP
					row_to_ar.append(config_path.split('_')[3]) # ALRP
					row_to_ar.append(config_path.split('_')[4]) # Viterbi factor
					...
					num_fakes = len(fakes.get(1))
					for i in range(len(seeds)):
						print 'calculating metrics for configuration {} user {}'.format(row_to_csv, i+1)
						seed = seeds[i]
						
						if config[2] != '84':
							seed_distance_covered = trace_distance(seed, False, locations_array)
						else:
							seed_distance_covered = trace_distance(seed, True, locations_array)
						std = numpy.std(seed)
						# fetching fakes
						for j in range(len(fakes.get(i+1))):
							fake = fakes.get(i+1)[j]
							if config[2] != '84':
								gen_distance = distance_bw_traces(seed, fake, False, locations_array)
								distance_covered = trace_distance(fake, False, locations_array)
								avg_gen_loss += gen_distance
								avg_dist_loss += distance_covered
							else:
								gen_distance = distance_bw_traces(seed, fake, True, locations_array)
								distance_covered = trace_distance(fake, True, locations_array)
								avg_gen_loss += gen_distance
								avg_dist_loss += distance_covered
							surge = surge_factor(fake, int(config[2]), locations_per_time, int(config[1]))
							avg_surge_loss += surge
							...
							avg_fake_std += (numpy.std(fake))
							rf_writer.writerow(row_to_csv)
							row_to_csv = row_to_csv[:-8]

						
						row_to_ar += [(i+1), avg_gen_loss/num_fakes, avg_dist_loss/num_fakes, avg_surge_loss/num_fakes, numpy.std(seed), avg_fake_std/num_fakes]
						
						ar_writer.writerow(row_to_ar)
						row_to_ar = row_to_ar[:-6]
		# break
